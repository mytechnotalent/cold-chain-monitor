 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Cold Chain Agricultural Environmental
 * Monitor status LED, button, servo, infrared decoder, the ChaCha20,
 * Poly1305, XChaCha20-Poly1305, BLAKE2b, and Argon2id security modules,
 * and the monitor input handlers that consume those peripherals.
 *
 * BRIEF:
...
static void assert_lamps(int red, int yellow, int green) {
    TEST_ASSERT_EQUAL_INT(red, mock_gpio_get(COLD_CHAIN_MONITOR_RED_LED_PIN));
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(COLD_CHAIN_MONITOR_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(COLD_CHAIN_MONITOR_GREEN_LED_PIN));
}
...
static void assert_servo_pulse(uint16_t expected) {
    TEST_ASSERT_EQUAL_UINT(expected,
                           mock_pwm_get_level(COLD_CHAIN_MONITOR_SERVO_PIN));
}
...
static void nec_arm(uint8_t command) {
    ...
    mock_gpio_timeline_begin_at(mock_timer_now_us(), s_ir_off, s_ir_lvl,
                                count, COLD_CHAIN_MONITOR_IR_PIN);
}

/**
 * @brief Verify initialization and all annunciator show states.
 *
 * @param void No parameters.
 * @return void
 */
static void test_status_led_show(void) {
    ...
    status_led_show(STATUS_LED_OFF);
    assert_lamps(0, 0, 0);
    status_led_show(STATUS_LED_NOMINAL);
    assert_lamps(0, 0, 1);
...
/**
 * @brief Verify the breach lamp is the only lamp lit.
 *
 * @param void No parameters.
 * @return void
 */
static void test_status_led_breach(void) {
    reset_pc();
    status_led_init();
    status_led_show(STATUS_LED_BREACH);
    assert_lamps(1, 0, 0);
}

/**
 * @brief Verify each temperature band maps to an annunciator state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_status_led_state(void) {
    reset_pc();
    TEST_ASSERT_EQUAL(STATUS_LED_OFF, status_led_state_for_temperature(0, false));
    TEST_ASSERT_EQUAL(STATUS_LED_BREACH,
                      status_led_state_for_temperature(0, true));
    TEST_ASSERT_EQUAL(STATUS_LED_WARNING,
                      status_led_state_for_temperature(-50, true));
    TEST_ASSERT_EQUAL(STATUS_LED_NOMINAL,
                      status_led_state_for_temperature(-51, true));
}
...
static void test_button_pressed(void) {
    ...
    gpio_put(COLD_CHAIN_MONITOR_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(button_pressed());
    gpio_put(COLD_CHAIN_MONITOR_BUTTON_PIN, false);
 ...
 * @return void
 */
static void test_button_consume(void) {
    reset_pc();
    button_init();
    gpio_put(COLD_CHAIN_MONITOR_BUTTON_PIN, true);
 ...
 * @return void
 */
static void test_button_debounce_elapsed(void) {
    reset_pc();
    button_init();
    gpio_put(COLD_CHAIN_MONITOR_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(button_consume_press());
...
/**
 * @brief Verify button_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(COLD_CHAIN_MONITOR_SERVO_PIN);
...
/**
 * @brief Verify set, seal, and vent drive the recorded PWM level.
 *
 * @param void No parameters.
 * @return void
 */
static void test_servo_actuate(void) {
    ...
    servo_vent();
    assert_servo_pulse(servo_angle_to_pulse_us(SERVO_ANGLE_VENTED_DEGREES));
    servo_seal();
    assert_servo_pulse(SERVO_MIN_PULSE_US);
}
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[COLD_CHAIN_MONITOR_IR_PIN]);
}
...
static void test_ir_poll_stuck_high(void) {
    ...
    ir_command_t cmd;
    reset_pc();
    gpio_put(COLD_CHAIN_MONITOR_IR_PIN, true);
    TEST_ASSERT_FALSE(ir_remote_poll(&cmd));
}
...
/**
 * @brief Verify a monitor button press re-seals the damper.
 *
 * @param void No parameters.
 * @return void
 */
static void test_monitor_button(void) {
    reset_pc();
    ...
    TEST_ASSERT_EQUAL_UINT(SERVO_MIN_PULSE_US,
                           mock_pwm_get_level(COLD_CHAIN_MONITOR_SERVO_PIN));
}

/**
 * @brief Verify a decoded infrared vent command vents the damper.
 *
 * @param void No parameters.
 * @return void
 */
static void test_monitor_ir_vent(void) {
    reset_pc();
    TEST_ASSERT_TRUE(monitor_init());
    button_release();
    servo_seal();
    nec_arm(MONITOR_IR_VENT_COMMAND);
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_EQUAL_UINT(servo_angle_to_pulse_us(SERVO_ANGLE_VENTED_DEGREES),
                           mock_pwm_get_level(COLD_CHAIN_MONITOR_SERVO_PIN));
}

/**
 * @brief Verify a decoded infrared seal command seals the damper.
 *
 * @param void No parameters.
 * @return void
 */
static void test_monitor_ir_seal(void) {
    reset_pc();
    TEST_ASSERT_TRUE(monitor_init());
    button_release();
    servo_vent();
    nec_arm(MONITOR_IR_SEAL_COMMAND);
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_EQUAL_UINT(SERVO_MIN_PULSE_US,
                           mock_pwm_get_level(COLD_CHAIN_MONITOR_SERVO_PIN));
}
...
/**
 * @brief Verify an unknown but valid remote command is named UNKNOWN.
 *
 * @param void No parameters.
 * @return void
 */
static void test_monitor_ir_unknown(void) {
    reset_pc();
    TEST_ASSERT_TRUE(monitor_init());
    button_release();
    servo_seal();
...
/**
 * @brief Run the status LED, button, and servo tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_status_led_breach);
    RUN_TEST(test_status_led_state);
...
/**
 * @brief Run the monitor peripheral integration tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_input_tests(void) {
    RUN_TEST(test_monitor_button);
    RUN_TEST(test_monitor_ir_vent);
    RUN_TEST(test_monitor_ir_seal);
    RUN_TEST(test_monitor_ir_unknown);
}
