#!/usr/bin/env python3
"""Generate the cold-chain-monitor-c-rp2350.png repository banner (1500x1500, pure stdlib).
...
def _draw_titles(banner, colors) -> None:
    ...
    None
    """
    banner.text("COLD CHAIN & AGRICULTURAL ENVIRONMENTAL MONITOR", 4,
                colors["gold"], banner.W // 2, 90)
    banner.text("OPERATION COLD IRON", 10, (0x0B, 0x10, 0x26),
                banner.W // 2 - 6, 176)
    banner.text("OPERATION COLD IRON", 10, colors["white"],
                banner.W // 2, 182)
    banner.text("RP2350  PICO 2", 5, colors["cyan"], banner.W // 2, 560)
    banner.text("-=[ COLD CHAIN DEFENSE + HACKING LAB ]=-", 3, colors["dim"],
                banner.W // 2, 660)
...
def _draw_features(banner, colors) -> None:
    ...
    None
    """
    banner.text("COLD_CHAIN_MONITOR", 6, colors["cyan"], banner.W // 2, 760)
    banner.text("RP2350", 5, colors["gold"], banner.W // 2, 880)
    banner.text("DHT11  LCD  IR  SERVO  LORA", 3, colors["silver"],
                banner.W // 2, 960)
    banner.text("DHT11  RYLR998  I2C LCD  VS1838B  SG90", 4,
                colors["silver"], banner.W // 2, 1160)
    banner.text("XCHACHA20-POLY1305  ARGON2ID  AUTHENTICATED", 3,
                colors["dim"], banner.W // 2, 1260)
    banner.text("AUTHENTICATED FREEZE ACTUATOR REPLY", 4, colors["gold"],
                banner.W // 2, 1350)
    banner.text("-=[ OPERATION COLD IRON  /  RP2350 ]=-", 3, colors["dim"],
                banner.W // 2, 1440)
...
def main() -> int:
    ...
    out = Path(__file__).resolve().parent.parent / "cold-chain-monitor-c-rp2350.png"
    banner.save(out)
